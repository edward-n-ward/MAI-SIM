...
'''
def acquisition_loop(stop_signal,stack,exposure):
    print('starting acquisition - Max')
    stop_signal.put(True)

    with nidaqmx.Task() as VoltageTask, nidaqmx.Task() as CameraTask, Bridge() as bridge:
        voltage = 0.95
        wait = 0.001
        VoltageTask.ao_channels.add_ao_voltage_chan('Dev1/ao0')
        CameraTask.do_channels.add_do_chan('Dev1/port0/line2')
        ...
        while core.get_remaining_image_count() == 0:
            time.sleep(.001)
        result = core.get_last_tagged_image()

        save_dir = 'C:/Users/SIM_Admin/Documents/Max/' + datetime.datetime.now().strftime('run%Y%m%dT%H%M') + '/'
        os.mkdir(save_dir)
        save_stack = np.zeros((100,result.tags['Height'],result.tags['Width'],1))
        i = 0
            ...
            if status == False:
                ...
                if stack.empty():
                    ...
                    if i % 10 == 0:
                        ...
                        time.sleep(exposure/100000)
                    CameraTask.write(False)
                    i += 1
                    # replace above with laser control, keep exposure time fixed

                    while core.get_remaining_image_count() == 0:
                        time.sleep(.001)
                    result = core.get_last_tagged_image()
                    pixels = np.reshape(result.pix,
                                        newshape=(result.tags['Height'],
                                                  result.tags['Width'],
                                                  -1))
                    save_stack[i%100] = pixels
                    if i % 100 == 0 and i != 0:
                        print('saving')
                        with open(save_dir+str(i//100),'wb') as f:
                            pickle.dump(save_stack,f)
